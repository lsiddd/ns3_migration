...
// TCP parameters
static
const uint32_t totalTxBytes = 1040;
...
main(int argc, char * argv[]) {
  ...
  for (uint32_t u = 0; u < ueNodes.GetN(); ++u) {
    if (!disableDl) {
      ...
      clientApps.Add(dlClient.Install(edgeNodes.Get(0)));
    }

    if (!disableUl) {
      ++ulPort;
      PacketSinkHelper ulPacketSinkHelper("ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), ulPort));
      serverApps.Add(ulPacketSinkHelper.Install(edgeNodes.Get(1)));
      UdpClientHelper ulClient(fogNodesAddresses[0], ulPort);
